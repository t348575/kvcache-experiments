"""Generate Pareto frontier plots from a pareto_measure.py results CSV.

Three figures:
  1. ttft_curves.png: measured f(N) cold-prefill and g(P) cache-hit TTFT curves
     with p05-p95 shading.
  2. pareto_frontier.png: speedup heatmap in (doc_size x prefix_fraction) space
     with the break-even contour (speedup = 1).
  3. concurrency.png: TTFT vs concurrency for cold and cache-hit, one line per
     doc size. Skipped if no concurrency sweep data is present.
...
from common.plot_common import configure_plots, save_figure, thousands_formatter, token_k_formatter

configure_plots()

try:
    from pareto_measure import SERVER_CONFIGS
except ImportError:
    SERVER_CONFIGS = {}
...
def available_cache_server_configs(rows: list[dict]) -> list[str]:
    ...
    if not configs:
        configs = {
            r["server_config"]
            for r in rows
            if r.get("curve") == "cold_prefill" and r.get("server_config")
        }
    return sorted(configs)


def is_storage_config(server_config: str | None) -> bool:
    if not server_config:
        return False
    return bool(SERVER_CONFIGS.get(server_config, {}).get("is_storage"))


def server_config_label(server_config: str) -> str:
    if is_storage_config(server_config):
        return f"{server_config} (storage)"
...
def select_cache_server_config(rows: list[dict], requested: str | None) -> str | None:
    ...
    for i, config in enumerate(configs, start=1):
        n_rows = sum(
            1
            for r in rows
            if r.get("curve") == "cache_hit" and r.get("server_config") == config
        )
        if n_rows == 0:
            n_rows = sum(
                1
                for r in rows
                if r.get("curve") == "cold_prefill" and r.get("server_config") == config
            )
        print(f"  {i}. {server_config_label(config)} ({n_rows} rows)")
...
def filter_cache_server_config(rows: list[dict], server_config: str | None) -> list[dict]:
    ...
    return [
        r for r in rows
        if r.get("curve") == "cold_prefill" or r.get("server_config") == server_config
    ]


def server_config_curve(rows: list[dict], curve: str, server_config: str | None) -> dict[int, dict]:
    return {
        r["doc_size"]: r
        for r in rows
        if r["curve"] == curve
        and r["concurrency"] == 1
        and (server_config is None or r.get("server_config") == server_config)
    }
...
def break_even_curve_values(
    ...
    for doc_size in doc_sizes:
        ...
        if kv_bpt and break_even_tokens > 0:
            break_even_ttft_s = float(f_interp(break_even_tokens))
            row["min_bandwidth_gbps"] = (
                break_even_tokens * kv_bpt / break_even_ttft_s / 1e9
                if break_even_ttft_s > 0
                else None
            )
        values.append(row)
    return values
...
def print_break_even_curve(values: list[dict[str, float | int | None]]) -> None:
    has_bandwidth = any(row.get("min_bandwidth_gbps") is not None for row in values)
    print("\nBreak-even curve values:")
    header = "  doc_tokens,prefix_tokens,prefix_fraction_pct"
    if has_bandwidth:
        header += ",min_bandwidth_gbps"
    print(header)
    for row in values:
        ...
        if prefix_tokens is None or prefix_frac is None:
            line = f"  {doc_size},none,none"
        else:
            line = f"  {doc_size},{prefix_tokens:.1f},{prefix_frac * 100:.2f}"
        if has_bandwidth:
            bw = row.get("min_bandwidth_gbps")
            line += f",{bw:.2f}" if bw is not None else ",none"
        print(line)


def minimum_break_even_bandwidth(
    values: list[dict[str, float | int | None]],
) -> dict[str, float | int] | None:
    candidates = [
        row
        for row in values
        if row.get("min_bandwidth_gbps") is not None
        and row.get("prefix_tokens") is not None
        and row.get("prefix_frac") is not None
    ]
    if not candidates:
        return None
    return min(candidates, key=lambda row: float(row["min_bandwidth_gbps"]))
...
def plot_pareto_frontier(
    ...
    break_even_values = break_even_curve_values(
        f_interp, g_interp, doc_sizes_measured, kv_bpt=kv_bpt
    )
    print_break_even_curve(break_even_values)
    min_break_even_bw = minimum_break_even_bandwidth(break_even_values)
    ...
    if min_break_even_bw:
        annotation = (
            "min break-even BW\n"
            f"{min_break_even_bw['min_bandwidth_gbps']:.2f} GB/s @ "
            f"{min_break_even_bw['doc_size'] / 1024:.0f}k doc, "
            f"{min_break_even_bw['prefix_frac'] * 100:.1f}% prefix"
        )
        ax.text(
            0.02,
            0.98,
        ...
        print(
            "  Minimum break-even bandwidth: "
            f"{min_break_even_bw['min_bandwidth_gbps']:.2f} GB/s "
            f"at doc_size={min_break_even_bw['doc_size']:.0f}, "
            f"prefix={min_break_even_bw['prefix_tokens']:.1f} tokens "
            f"({min_break_even_bw['prefix_frac'] * 100:.2f}%)"
        )
...
def plot_merged_pareto_frontier(
    rows: list[dict],
    csv_path: str,
    ...
    cold_pr, _ = load_per_run_curves(csv_path, None)
    cold = cold_pr or serial_curve(rows, "cold_prefill")
    if not cold:
        print("  No cold_prefill serial data found — skipping merged Pareto plot.")
        return

    doc_sizes_measured = sorted(cold.keys())
    doc_arr = np.array(doc_sizes_measured, dtype=float)
    prefix_fracs = np.linspace(0.0, 1.0, 200)
    cold_ttft_arr = np.array([cold[d]["ttft_mean_s"] for d in doc_sizes_measured])
    f_interp = build_interpolator(cold)

    x_idx = np.arange(len(doc_sizes_measured))
    x_labels = [f"{int(d) // 1024}k" for d in doc_sizes_measured]
    colors = plt.cm.tab10(np.linspace(0, 1, max(len(server_configs), 1)))

    fig, ax = plt.subplots(figsize=(8, 5))
    fig.suptitle(f"KV-Cache break-even frontiers ({model})", fontweight="bold")
    legend_handles = []
    bandwidth_lines = []

    for config, color in zip(server_configs, colors):
        _, hit_pr = load_per_run_curves(csv_path, config)
        hit = hit_pr or server_config_curve(rows, "cache_hit", config)
        if not hit:
            hit = server_config_curve(rows, "cold_prefill", config)
        if not hit:
            print(f"  No cache_hit data for {config} — skipping in merged Pareto plot.")
            continue
        ...
        if kv_bpt:
            ...
            min_bw = minimum_break_even_bandwidth(break_even_values)
            if min_bw:
                bandwidth_lines.append(
                    f"{server_config_label(config)}: {min_bw['min_bandwidth_gbps']:.2f} GB/s"
                )
        speedup_mat = np.zeros((len(prefix_fracs), len(doc_arr)))
        for i, frac in enumerate(prefix_fracs):
            cached = np.maximum(0, g_interp(frac * doc_arr)) + np.maximum(
        ...
        legend_handles.append(
            Line2D([0], [0], color=color, linewidth=2, label=server_config_label(config))
        )
...
def main():
    ...
    p.add_argument(
        "--merged-pareto",
        action="store_true",
        help="Also generate merged_pareto_frontier.png with break-even lines for all cache server configs.",
    )
    ...
    if cache_server_config:
        ...
        print(f"  Using cache_hit server_config: {server_config_label(cache_server_config)}")
    ...
    if cold_pr and hit_pr:
        ...
        if not hit and cache_server_config:
            hit = server_config_curve(rows, "cold_prefill", cache_server_config)
    ...
    if args.layers and args.kv_heads and args.head_dim:
        ...
        print(f"\n  KV bytes/token: {kv_bpt:,}")

    if args.merged_pareto:
        merged_configs = available_cache_server_configs(load_results(args.results))
        plot_merged_pareto_frontier(
            load_results(args.results),
            args.results,
            merged_configs,
            args.model,
            os.path.join(args.output_dir, "merged_pareto_frontier.png"),
            kv_bpt=kv_bpt,
        )
    ...
    if kv_bpt:
        print("  Generating IO budget plot…")
