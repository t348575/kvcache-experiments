...
async def main(args):
    ...
    warmup_request_stats = []
    benchmark_request_stats = []
    warmup_start_time = warmup_end_time = benchmark_start_time = benchmark_end_time = time.time()
    ...
    if args.warmup_only:
        write_resp("------warmup only mode, skipping query round------\n")
        ...
        if not warmup_df.empty:
            ...
            warmup_df["latency"] = warmup_df["request_end"] - warmup_df["request_start"]
            warmup_df.to_csv("warmup_round.csv", index=False)
            if args.csv_output:
                warmup_df["round"] = "warmup"
                warmup_df.to_csv(args.csv_output, index=False)
        return
    ...
    if not warmup_df.empty:
        ...
        warmup_df = pd.DataFrame(columns=["prompt_id", "request_start", "ttft",
                                           "request_end", "successful", "is_miss",
                                           "latency", "ttft_time"])
    ...
    if not warmup_df.empty:
        warmup_df["latency"] = warmup_df["request_end"] - warmup_df["request_start"]
    benchmark_df["latency"] = benchmark_df["request_end"] - benchmark_df["request_start"]
    ...
    if args.csv_output:
        warmup_df["round"] = "warmup"
        benchmark_df["round"] = "query"
        combined_df = pd.concat([warmup_df, benchmark_df], ignore_index=True)
        combined_df.to_csv(args.csv_output, index=False)

    # Print results
    warmup_mean_ttft = warmup_df.query("successful == True")["ttft"].mean() if not warmup_df.empty else float("nan")
    query_mean_ttft = benchmark_df.query("successful == True")["ttft"].mean()
    warmup_success_count = warmup_df.query("successful == True").shape[0] if not warmup_df.empty else 0
...
def create_argument_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to write per-request TTFT and latency for every warmup and query request.",
    )

    parser.add_argument(
        "--warmup-only",
        action="store_true",
        help="Only run the warmup round; skip the query round.",
    )

    parser.add_argument(
        "--query-only",
        action="store_true",
        help="Only run the query round; skip the warmup round.",
    )
